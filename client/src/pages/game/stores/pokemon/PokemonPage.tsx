    ...
    return (
        <div
            ...
            className={`
                ...
                ${isDragging ? 'opacity-50 scale-95 border-yellow-500' : 'border-slate-800 hover:border-yellow-500/50 hover:bg-slate-800'}
            `}
        >
...
export default function PokemonPage() {
    ...
    const handleDragEnd = (event: DragEndEvent) => {
        ...
        if (over && active.id !== over.id) {
            const oldIndex = party.findIndex((pkm) => pkm._id === active.id);
            const newIndex = party.findIndex((pkm) => pkm._id === over.id);
            reorderParty(oldIndex, newIndex);
        }
    };
